...
def extract_shortest_reads(fasta_path, output_path, seq_type, base_name, direction):
    ...
    if seq_type=="merged":
        ...
        if not seen_ont: os.remove(ont_out)
        if not seen_hifi: os.remove(hifi_out)
    else:
        out_h.close()
...
def main():
    ...
    p.add_argument('--dir_IN_L',required=True)
    p.add_argument('--dir_IN_R',required=True)
    p.add_argument('--flye',action='store_true')
    p.add_argument('--assemble','-a',action='store_true')
    p.add_argument('-t','--threads',type=int,default=20)
    p.add_argument('--min_overlap',type=int,default=50)
    p.add_argument('--error_rate',type=float,default=0.15)
    p.add_argument('--kmer_size',type=int,default=15)
    p.add_argument('-L','--lgsreads',required=True)
    p.add_argument('-W','--wgs1',required=True)
    p.add_argument('-w','--wgs2',required=True)
    p.add_argument('-N','--NextPolish',required=True)
    args=p.parse_args()
