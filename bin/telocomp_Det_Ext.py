...
def process_bam_overhang(bam_path, outdir, motifs, logger):
    ...
    for (rn, side), reads in buckets.items():
        for r in reads:
            ...
            if aligned / len(r['seq']) < 0.2 or aligned < 10000:
                continue
            seq, c = r['seq'], r['clip']
...
def main():
    ...
    process_bam_overhang(args.ont,  ont_out, motifs, log)
    process_bam_overhang(args.hifi, hifi_out, motifs, log)
